...
(define-test-suite red-black-tree-tests
   ...
   (test "red-black-tree? returns true for red-black-tree"
      (assert-true (red-black-tree? (make-red-black-tree :less-than? <))))

   (test "make-red-black-tree raises an invalid argument exception if less-than? is not a procedure"
      (assert-exception-thrown (make-red-black-tree :less-than? #f)
         &invalid-argument-exception))

   (test "a newly created red-black-tree has size 0"
      (assert-equal? (red-black-tree-size (make-red-black-tree :less-than? <)) 0))

   (test "a red-black-tree with a single item inserted has size 1"
      (let ((tree (make-red-black-tree :less-than? <)))
         (red-black-tree-insert! tree 2)
         (assert-equal? (red-black-tree-size tree) 1)))

   (test "(red-black-tree :less-than? < 1 2 3 4) contains 3"
      (let ((t (red-black-tree :less-than? < 1 2 3 4)))
   ...
   (test "inserting 1 2 3 into a red black tree results in tree with 2 as the root
      ...
      (let ((tree::%red-black-tree (make-red-black-tree :less-than? <)))
   ...
   (test "empty tree is balanced"
      (assert-true (red-black-tree-balanced? (make-red-black-tree :less-than? <))))
